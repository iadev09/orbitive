...
pub const CACHE_MUTATION_RING_SPEC: RingSpec =
    RingSpec::per_node(CACHE_MUTATION_RING_CAPACITY, CACHE_MUTATION_RING_PAYLOAD_CAPACITY);
pub const CACHE_PAYLOAD_RING_SPEC: RingSpec =
    RingSpec::per_node(CACHE_PAYLOAD_RING_CAPACITY, CACHE_PAYLOAD_RING_PAYLOAD_CAPACITY);
...
impl<L: CacheLayout> OrbitTyped for MutationRecord<L> {
    ...
    const RING_SPEC: RingSpec =
        RingSpec::per_node(L::MUTATION_RING_SPEC.capacity, L::MUTATION_RING_SPEC.payload_capacity);
}
...
impl<L: CacheLayout> OrbitTyped for PayloadRecord<L> {
    ...
    const RING_SPEC: RingSpec =
        RingSpec::per_node(L::PAYLOAD_RING_SPEC.capacity, L::PAYLOAD_RING_SPEC.payload_capacity);
}

pub(crate) fn validate<L: CacheLayout>() -> crate::Result<()> {
    if L::MUTATION_RING_KIND == L::PAYLOAD_RING_KIND {
        return Err(crate::Error::InvalidLayout("mutation and payload ring kinds must differ"));
    }
    ...
    if L::MUTATION_RING_SPEC.capacity == 0 || L::PAYLOAD_RING_SPEC.capacity == 0 {
        return Err(crate::Error::InvalidLayout("ring capacities must be greater than zero"));
    }
    if !L::MUTATION_RING_SPEC.capacity.is_power_of_two()
        || !L::PAYLOAD_RING_SPEC.capacity.is_power_of_two()
    {
        return Err(crate::Error::InvalidLayout("ring capacities must be powers of two"));
    }
    ...
    if L::PAYLOAD_RING_SPEC.payload_capacity == 0 {
        return Err(crate::Error::InvalidLayout("payload slots must hold at least one byte"));
    }
    Ok(())
}
