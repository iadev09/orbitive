...
impl fmt::Display for Error {
    fn fmt(
        ...
        match self {
            ...
            Self::EmptyFleet => {
                write!(f, "fleet_capacity must be ≥ 1; Orbit needs at least one node lane")
            }
            Self::NodeOutsideFleet { node_id, fleet_capacity } => {
                write!(f, "node_id {node_id} is outside fleet_capacity {fleet_capacity}")
            }
